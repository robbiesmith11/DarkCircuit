...
def _setup_ssh_connection(host: str, port: int, username: str, password: Optional[str] = None,
    ...
    try:
        ...
        # make sure our PROMPT_READY_MARKER is in ~/.bashrc exactly once
        # Simplified approach to ensure our PROMPT_READY_MARKER is in ~/.bashrc
        cmd = '''
        if ! grep -q "CMD_READY" ~/.bashrc; then
            echo '' >> ~/.bashrc
            echo '# This marker is used by the SSH application to detect command completion' >> ~/.bashrc
            echo 'export PROMPT_COMMAND='"'"'printf "\\033]1337;CMD_READY\\007"'"'"'' >> ~/.bashrc
        fi
        '''
        print(cmd)
        ...
        if exit_status != 0:
            ...
            raise RuntimeError(f"Failed to update .bashrc: {err}")
