...
class Darkcircuit_Agent:
    def __init__(self, model_name="gpt-4o-mini", reasoning_prompt=None, response_prompt=None):
        ...
        # Create chat prompt templates with history placeholders
        self.reasoning_template = ChatPromptTemplate.from_messages([
            ("system", reasoning_prompt or DEFAULT_REASONING_PROMPT),
            MessagesPlaceholder(variable_name="history"),
            MessagesPlaceholder(variable_name="input")
        ])

        self.response_template = ChatPromptTemplate.from_messages([
            ("system", response_prompt or DEFAULT_RESPONSE_PROMPT),
            MessagesPlaceholder(variable_name="history"),
            MessagesPlaceholder(variable_name="input")
        ])
    ...
    async def reasoner(self, state: MessagesState):
        ...
        try:
            ...
            for msg in state["messages"]:
                ...
                elif getattr(msg, "type", "") in ["human", "system", "ai"]:
                    filtered_messages.append(msg)
    ...
    async def responder(self, state: MessagesState):
        ...
        try:
            # Filter messages for the responder - ONLY include human, system, and simple AI messages without tool calls
            filtered_messages = []
            human_message = None
            ...
            for msg in state["messages"]:
                if isinstance(msg, HumanMessage):
                    ...
                    # Only include string-typed AI messages without tool_calls
                    if not hasattr(msg, "tool_calls") or not getattr(msg, "tool_calls", []):
                        filtered_messages.append(msg)
    ...
    async def run_agent_streaming(self, prompt: str, messages=None):
        """
        Run the agent with streaming output using structured message history.

        Args:
            prompt: The last user query (for backward compatibility)
            messages: Optional list of messages representing the conversation history
        """
        # Create properly structured input messages
        if messages is None:
            # Default to single message if no history provided
            input_messages = [HumanMessage(content=prompt)]
        else:
            # Process messages to ensure valid tool call/response pairs
            processed_messages = []
            tool_call_map = {}
            tool_response_map = {}

            # First pass: collect all tool call IDs and their responses
            for msg in messages:
                # Collect tool responses
                if isinstance(msg, ToolMessage):
                    tool_id = getattr(msg, "tool_call_id", None)
                    if tool_id:
                        tool_response_map[tool_id] = msg

                # Look for AIMessages with tool_calls
                if isinstance(msg, AIMessage) and hasattr(msg, "tool_calls"):
                    for tool_call in getattr(msg, "tool_calls", []):
                        call_id = tool_call.get("id")
                        if call_id:
                            tool_call_map[call_id] = tool_call

            # Second pass: build valid sequence of messages
            for msg in messages:
                if isinstance(msg, HumanMessage) or isinstance(msg, SystemMessage):
                    ...
                    if hasattr(msg, "tool_calls") and msg.tool_calls:
                        # For AI messages with tool calls, ensure we only include valid ones
                        valid_tool_calls = []
                        for tool_call in msg.tool_calls:
                            call_id = tool_call.get("id")
                            if call_id in tool_response_map:
                                valid_tool_calls.append(tool_call)

                        if valid_tool_calls:
                            # Create a new AI message with only valid tool calls
                            new_msg = AIMessage(content=msg.content)
                            setattr(new_msg, "tool_calls", valid_tool_calls)
                            processed_messages.append(new_msg)

                            # Immediately after, include all corresponding tool responses
                            for tool_call in valid_tool_calls:
                                call_id = tool_call.get("id")
                                if call_id in tool_response_map:
                                    processed_messages.append(tool_response_map[call_id])
                        else:
                            # If no valid tool calls, include as regular AI message
                            # Strip any tool_calls attribute to be safe
                            new_msg = AIMessage(content=msg.content)
                            processed_messages.append(new_msg)
                    else:
            ...
            # Add the latest human message if not already present
            latest_human_message_added = False
            for msg in reversed(processed_messages):
                if isinstance(msg, HumanMessage):
                    latest_human_message_added = True
                    break

            if not latest_human_message_added:
                processed_messages.append(HumanMessage(content=prompt))

            # Before finalizing, do one more safety check to remove any AI messages with tool_calls that don't have responses
            final_messages = []
            for msg in processed_messages:
                if isinstance(msg, AIMessage) and hasattr(msg, "tool_calls") and getattr(msg, "tool_calls"):
                    # Double-check that ALL tool calls have corresponding responses
                    all_calls_have_responses = True
                    for tool_call in getattr(msg, "tool_calls"):
                        call_id = tool_call.get("id")
                        if call_id not in tool_response_map:
                    ...
                    if all_calls_have_responses:
                        ...
                        # Convert to a regular AI message without tool_calls
                        final_messages.append(AIMessage(content=msg.content))
                else:
                    final_messages.append(msg)

            # Use the safe, final messages
            processed_messages = final_messages

            # Limit to max 10 messages to prevent exceeding context limits
            if len(processed_messages) > 10:
                # Keep a balanced selection, prioritizing recent ones
                processed_messages = processed_messages[-10:]

            input_messages = processed_messages
            print(
                f"[Agent] Message processing complete: {len(input_messages)} messages, {len(tool_call_map)} tool calls, {len(tool_response_map)} tool responses")

        # Log the messages being sent to the agent
        print(f"[Agent] Using {len(input_messages)} structured messages")
        for i, msg in enumerate(input_messages):
            msg_type = type(msg).__name__
            content = getattr(msg, "content", "")[:50]
            if hasattr(msg, "tool_calls") and getattr(msg, "tool_calls"):
                tool_calls_info = f" with {len(getattr(msg, 'tool_calls'))} tool_calls"
            else:
                tool_calls_info = ""
            print(f"[Agent] Message {i}: {msg_type}{tool_calls_info} - {content}...")
    ...
    def create_retrieval_chain(self, retriever):
        ...
        """
        # Create a custom prompt that includes conversation history
        prompt = ChatPromptTemplate.from_messages([
            ("system", "You are an assistant that helps with retrieving information."),
